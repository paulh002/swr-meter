...
void swr_alarm_threshold_menu(void)
{
  ...
  {
    if ((Enc.getCount()/ENC_RESDIVIDE > 0) && (R.SWR_alarm_trig < 40) )  // SWR 4:1 is MAX value, and equals no alarm
    {
      R.SWR_alarm_trig++;
    }
    if ((Enc.getCount()/ENC_RESDIVIDE < 0) && (R.SWR_alarm_trig > 15) )  // SWR of 1.5:1 is MIN value for SWR alarm
    {
      R.SWR_alarm_trig--;
    }
    // Reset data from Encoder
...
void displaydim_menu(void)
{
  ...
  if (Enc.getCount()/ENC_RESDIVIDE != 0)
  {
    if ((Enc.getCount()/ENC_RESDIVIDE > 0) && (R.disp.tft_backlight < 10) )  // 10 is max value
    {
      R.disp.tft_backlight++;
    }
    if ((Enc.getCount()/ENC_RESDIVIDE < 0) && (R.disp.tft_backlight > 0) )  // 0 is min value
    {
      R.disp.tft_backlight--;
    }
    // Reset data from Encoder
...
void shutoffvoltage_menu(void)
{
  ...
  if (Enc.getCount()/ENC_RESDIVIDE != 0)
  {
    if ((Enc.getCount()/ENC_RESDIVIDE > 0) && (R.loBatt < 40) )  // 4.0 Volts is max value
    {
      R.loBatt++;
    }
    if ((Enc.getCount()/ENC_RESDIVIDE < 0) && (R.loBatt > 33) )  // 3.3 Volts is min value
    {
      R.loBatt--;
    }
    // Reset data from Encoder
...
void calibrate_menu(void)
{
  ...
  if (Enc.getCount()/ENC_RESDIVIDE != 0)
  {
    if ((Enc.getCount()/ENC_RESDIVIDE > 0) && (current_selection < 250) )
    {
      current_selection++;
    }
    if ((Enc.getCount()/ENC_RESDIVIDE < 0) && (current_selection > 10) )
    {
      current_selection--;
    }
    // Reset data from Encoder
