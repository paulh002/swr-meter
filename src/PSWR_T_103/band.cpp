...
void display_band()
{
  ...
  if (first_B == 1)
  {
    VirtLCDb.init(20,2,10,205, FONT_12, BAND_TEXT_COLOR, ILI9341_BLACK); 
    VirtLCDb.clear();
    first_B = 0;
  } 
 ...
 */
  ...
  freq = CAT.GetFA() ;
  for (int i; i < nobands; i++)
    {
    if (freq >= freqswitch_low[i] && freq <= freqswitch_high[i])
      {
      if (f_band != i)
        {
        set_band(i);
