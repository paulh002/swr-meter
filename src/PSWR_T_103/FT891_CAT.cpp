...
bool FT891_CAT::GetMessage ()
{
  ...
  if (CatTelClient.connected() && CatTelClient.available())
  {  
  CatTelClient.read((const uint8_t *)rxBuff, sizeof ( rxBuff )); 
 ...
 */
    ...
    if (CatTelClient.connected())
    {
    Serial.println ("Send data ");
    if ( addNewline )
      {
        CatTelClient.write ( (const uint8_t *)txBuff[txBuffIndex], strlen(txBuff[txBuffIndex]) );   // Send the message (debugging mode)
        CatTelClient.write ( (const uint8_t *)"\r\n", strlen("\r\n") );   // Send the message (debugging mode)      
      }
    else
        CatTelClient.write ( (const uint8_t *)txBuff[txBuffIndex], strlen(txBuff[txBuffIndex]) );   // Send the message with no newline
    }
    else
    {
